...
fetch_coo_secrets() {
  ...
  (
    umask 077
    {
      ...
      [ -n "$r2_access_key_id" ]      && echo "export R2_TRANSCRIPTS_ACCESS_KEY_ID='$r2_access_key_id'"
      [ -n "$r2_secret_access_key" ]  && echo "export R2_TRANSCRIPTS_SECRET_ACCESS_KEY='$r2_secret_access_key'"
      [ -n "$age_identity" ]          && echo "export TRANSCRIPTS_AGE_IDENTITY='$age_identity'"
    } > "$env_file"
  )
  chmod 600 "$env_file"
  ...
  [ -n "$r2_access_key_id" ]      && export R2_TRANSCRIPTS_ACCESS_KEY_ID="$r2_access_key_id"
  [ -n "$r2_secret_access_key" ]  && export R2_TRANSCRIPTS_SECRET_ACCESS_KEY="$r2_secret_access_key"
  [ -n "$age_identity" ]          && export TRANSCRIPTS_AGE_IDENTITY="$age_identity"
  return 0
}
